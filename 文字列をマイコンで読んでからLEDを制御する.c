文字列をマイコンで読んでからLEDをONにする

①どのビットを操作するか USART2は17ビット
クロック供給で電源供給
RCC->APB1ENR |= ( 1 << 17);

②電源を入れたらGPIOからUSART2の送受信のラインにする
ピン2とピン3のモードを代替機能モード(Alternate function mode)にする
MODER2[1:0]を10にする
MODER3[1:0]を10にする

GPIOA->MODER &= ~(0xF << 4); //PA2,PA3の領域を0000にクリアする
GPIOA->MODER |= (0xA << 4);  //PA2,PA3の領域に0xAを書き込む　4は16進数で1010

AF領域のAFRL2とAFRL3に入れる
...
void usart2_init(void){
	...
	USART2->CR1 |= (1 << 13)|(1 << 3)|(1 << 2) //13ビット目のUSART2の電源、3ビット目の送信、2ビット目の受信をON
}
//1文字送信関数の定義
...
int main (void){
    ...
    char input_buffer[] = "led on"; 

    while(1) {
        // もし入力が "led on" だったらLEDを点灯
